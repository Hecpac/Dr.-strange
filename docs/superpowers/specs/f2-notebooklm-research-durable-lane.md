...
**Result-count rule (fail-closed on zero/missing).** Applies on both the happy
path and recovery: a `result_json` with `imported_count > 0` →
`applied` / `verified_applied` → job completed; a `result_json` with
`imported_count == 0` or missing → `blocked_manual_review` for this round. We do
not silently complete a research that imported nothing. A separate
`completed_no_imports` semantic is explicitly out of scope here.
...
Job ↔ effect coupling:
- Effect `applied` / `verified_applied` → job `completed` (only with
  `imported_count > 0`; see §5 result-count rule).
- Effect `verified_absent` → **controlled retry of the existing row** (the unique
  idempotency key means re-recording does not create a new row). The executor
  transitions `verified_absent → apply_in_progress` and increments
  `attempt_count`; the adapter call is permitted **only after** that committed
  transition. Retries are bounded by the job's `max_attempts`; on exhaustion →
  `blocked_manual_review`.
- Effect `blocked_manual_review` → the runner fails the job via
  `JobService.fail(job_id, error="effect_blocked_manual_review", retry=False)`
  (never retryable). The `result`/metadata carries `external_effect_id`,
  `idempotency_key`, `notebook_id`, `effect_kind`, and `verifier_reason`. Plus an
...
4. Executor records **result**; commit. `imported_count > 0` → `applied`;
   `imported_count == 0` or missing → `blocked_manual_review` (§5 result-count
   rule, §7 handling).
5. On `applied`, the runner completes the job. (No verifier on the happy path.)
