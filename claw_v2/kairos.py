...
class KairosService:
    ...
    def _handle_auto_publish_social(self, decision: TickDecision, trace_context: dict[str, Any] | None = None) -> None:
        ...
        if self._already_published_social(page_source):
    ...
    def _already_published_social(self, source: str) -> bool:
        ...
        Keyed on the stable wiki source filename, NOT the tweet text: the wording
        is regenerated by a nondeterministic LLM each tick, so an exact-text match
        never fires. Mirrors `_recent_duplicate_approval_notification`: looks at
        recent success events so a re-decide of auto_publish_social does not double-post.
        """
        try:
        ...
        for event in events:
            ...
            if not isinstance(payload, dict):
                continue
            if payload.get("success") and str(payload.get("source") or "") == source:
                return True
        return False
