...
invariants:
  ...
  b45d_failure_summary_is_registry_invoked:
    rule: 'B4.5d. operational_failure_summary is invoked through registry
          Route data ONLY in the _handle_text_body text chain: it runs
          dispatch_routes(self._failure_summary_slot, route_ctx,
          on_decision=self._emit_route_decision), a one-Route per-slot bridge
          (Route(OPERATIONAL_FAILURE_SUMMARY_MATCHER.name,
          self._route_operational_failure_summary)) at the ORIGINAL §5.1 row
          5 slot, after pending_tasks and before the B4.5b operational-status
          bridge. It must NOT join _pre_brain_routes. The adapter is pure:
          it calls the legacy gate with ctx.stripped + ctx.session_id, takes
          both reason slugs from OPERATIONAL_FAILURE_SUMMARY_MATCHER, and
          propagates store_memory_limit=3000. The text-chain quality guard
          stays AT THE CALL SITE after dispatch and before
          _post_capture_intercepted (source="operational_failure_summary",
          assistant_limit=outcome.store_memory_limit), preserving
          dispatch_decision before a possible quality_guard_triggered.
          handle_multimodal is explicitly NOT migrated: its direct gate,
          inline dispatch_decision, quality guard, store/remember and return
          remain LEGACY and AST-locked by the B4.4d pilot. The full-chain
          collision winner and A1 ownership flip remain unchanged.'
    chokepoints:
      - bot.BotService.__init__  # _failure_summary_slot construction
      - bot.BotService._route_operational_failure_summary
      - bot.BotService._handle_text_body
      - bot.BotService.handle_multimodal  # legacy, must not migrate in B4.5d
    enforced_by:
      - tests/test_b45d_failure_summary_route_registry.py
      - tests/test_b44d_failure_summary_matcher_pilot.py
      - tests/test_botservice_migration_rails.py
      - tests/test_telegram_imperative_router.py::test_failure_summary_wins_status_collision_and_early_returns
      - tests/test_telegram_imperative_router.py::test_stop_marker_flips_status_collision_to_operational_status
    why: Fourth per-slot migration closes the guarded compound-route case
         while proving invocation ownership can split by entry point: the
         text chain moves to Route data without silently migrating multimodal.
  ...
  b45b_operational_status_is_registry_invoked:
    rule: 'B4.5b. The operational_status route is invoked through registry
          ...
          after the B4.5d failure-summary per-slot bridge, before the B4.5a
          cleanup bridge. The route must NOT join _pre_brain_routes
  ...
  b45a_cleanup_status_is_registry_invoked:
    rule: 'B4.5a. The cleanup_status route is invoked through registry Route
          ...
          the B4.5e owner-delegation per-slot bridge. The route must NOT join
          _pre_brain_routes (the early registry slot, rows 2-3): that would
          move interception ahead of pending-tasks/failure-summary/
          operational-status and reorder consolidated dispatch_decision
          rows — order is behavior (§5.1). The adapter takes its reason
          slugs from CLEANUP_STATUS_MATCHER (byte-identical telemetry);
          capture side-effects go through _post_capture_intercepted (the
          same two calls the legacy call site made). EXPECTED_PRE_BRAIN_ORDER
          no longer lists the direct call (deliberate 19->18 edit, same
          commit as §5.1); the bridge slot is locked by the b45a test via
          AST position between its legacy neighbors.'
    chokepoints:
      - bot.BotService._route_cleanup_status
      - bot.BotService.__init__  # _cleanup_status_slot construction
    enforced_by:
      - tests/test_b45a_cleanup_route_registry.py
      - tests/test_botservice_migration_rails.py
    why: B4.5 moves handlers INTO the registry one route at a time; the
         first migration must prove the per-slot bridge shape — registry
         invocation without order drift — before any route joins the early
         slot or the slots consolidate.
  ...
  b44d_failure_summary_matcher_is_declarative_data:
    rule: 'B4.4d. The operational-failure-summary route match contract is
          ...
          for the gate in
          BotService._maybe_handle_operational_failure_summary, the Route
          adapter/slot that owns _handle_text_body since B4.5d, and the
          inline dispatch_decision slugs in legacy handle_multimodal; bot.py
          carries no parallel recognizer (the staticmethod is deleted).
          Decisions are
          ...
          extraction itself never moved calls. B4.5d later migrates ONLY the
          text-chain invocation into dispatch_routes and deliberately edits
          EXPECTED_PRE_BRAIN_ORDER + §5.1; handle_multimodal remains legacy.
          Per the Routing Contract the predicate reads literal message text
          only — no session_state, no ledger.'
    chokepoints:
      - dispatch.matchers.OPERATIONAL_FAILURE_SUMMARY_MATCHER
      - bot.BotService._maybe_handle_operational_failure_summary
      - bot.BotService._route_operational_failure_summary
      - bot.BotService.handle_multimodal
    enforced_by:
      - tests/test_b44d_failure_summary_matcher_pilot.py
      - tests/test_b45d_failure_summary_route_registry.py
      - tests/test_telegram_imperative_router.py::test_failure_summary_wins_status_collision_and_early_returns
  ...
  b44e_owner_delegation_matcher_is_declarative_data:
    rule: 'B4.4e. The owner-delegation boolean route contract is DATA:
          dispatch.matchers.OWNER_DELEGATION_MATCHER (frozen RouteMatcher —
          the boolean projection of the existing rich-intent classifier plus
          byte-identical owner_delegation_matched / owner_delegation_no_match
          base dispatch reason slugs) is the single source for the gate in
          BotService._maybe_handle_owner_delegation_request and the static
          outcomes returned by the B4.5e registry adapter at the original,
          order-locked _handle_text_body slot. The gate asks matcher.match(text)
          first, then calls detect_owner_delegation(text) only to materialize
          OwnerDelegationIntent fields needed by objective resolution and the
          inner dynamic reason/matched_pattern, both of which remain exactly
          owner_delegation:{intent.kind}. That second call is not a parallel
          boolean route recognizer. The other direct detector consumers —
          bot._looks_like_operator_action_request,
          bot_helpers.looks_like_actionable_telegram_message, and
          semantic_turn.classify_semantic_turn — remain direct because they
          perform actionability/semantic classification or read rich fields;
          they emit no owner route decision, and the pilot AST-locks this
          complete bounded consumer set. Decisions are old-vs-new
          corpus-locked against an independent frozen legacy copy of all six
          pattern tuples, normalization, 0.70 threshold/scoring, and the three
          ordered loops. Cross-matcher rails include real collisions with
          operational-status and failure-summary plus the A1 stop-marker flip.
          Matcher extraction itself did not move the legacy call. B4.5e later
          migrates ONLY the text-chain invocation into its per-slot registry
          bridge and deliberately edits EXPECTED_PRE_BRAIN_ORDER + §5.1; the
          adapter keeps matcher slugs and store_memory_limit=4000, while the
          call site remains guard-free. Per the Routing Contract matcher.match
          reads literal text only — no session_state, reply context, or ledger.'
    chokepoints:
      - dispatch.matchers.OWNER_DELEGATION_MATCHER
      - bot.BotService._maybe_handle_owner_delegation_request
      - bot.BotService._handle_text_body
      - bot.BotService._route_owner_delegation
      - bot._looks_like_operator_action_request  # actionability, not route ownership
      - bot_helpers.looks_like_actionable_telegram_message  # actionability only
      - semantic_turn.classify_semantic_turn  # rich semantic consumer
    enforced_by:
      - tests/test_b44e_owner_delegation_matcher_pilot.py
      - tests/test_b45e_owner_delegation_route_registry.py
      - tests/test_owner_delegation_routing.py
      - tests/test_botservice_migration_rails.py
      - tests/test_telegram_imperative_router.py::test_owner_delegation_never_falls_back
      - tests/test_telegram_imperative_router.py::test_english_owner_delegation_with_options_never_falls_back
    why: Fifth application of the declarative matcher shape, and the first
         backed by a rich classifier whose structured result must remain
  ...
  b44f_actionable_task_matcher_is_declarative_data:
    rule: 'B4.4f. The direct actionable-task literal match contract is DATA:
          ...
          dispatch_decision name/reason slugs at the order-locked
          _handle_text_body row 8 registry bridge introduced by B4.5f.
          Matcher extraction itself did not move the call; B4.5f later moved
          only its invocation without moving runtime/channel gates,
          session_state, follow-up, semantic-turn, continuation resolution,
          ...
          full-chain collision. The B4.4f extraction added no Route, adapter,
          slot, dispatch_routes, _pre_brain_routes or order change; registry
          invocation remained the separate B4.5f slice.'
    chokepoints:
      ...
      - bot.BotService._handle_text_body
      - bot.BotService._route_actionable_task
    enforced_by:
  ...
  b45e_owner_delegation_is_registry_invoked:
    rule: 'B4.5e. owner_delegation is invoked through registry Route data
          ONLY in the _handle_text_body text chain: dispatch_routes(
          self._owner_delegation_slot, route_ctx,
          on_decision=self._emit_route_decision) runs exactly once at the
          ORIGINAL §5.1 row 7 slot, after the B4.5a cleanup bridge and before
          _maybe_handle_telegram_imperative_request. The single Route is
          Route(OWNER_DELEGATION_MATCHER.name,
          self._route_owner_delegation); it must NOT join _pre_brain_routes.
          The pure adapter passes ctx.stripped, ctx.session_id and
          ctx.runtime_channel to the unchanged legacy rich-intent handler.
          None becomes fall_through with the matcher unmatched slug; capture
          becomes intercepted with the matcher matched slug and
          store_memory_limit=4000. There is no quality guard: capture goes
          directly through _post_capture_intercepted using the outcome limit.
          Telemetry cardinality/order is load-bearing. MATCH emits exactly
          TWO dispatch decisions: first the handler internal dynamic decision
          (handler owner_delegation, intercepted, reason and matched_pattern
          owner_delegation:{intent.kind}, captured true), then the registry
          callback static decision (handler owner_delegation, intercepted,
          reason owner_delegation_matched, captured true, NO matched_pattern).
          NO MATCH emits exactly ONE registry callback decision
          (fall_through, owner_delegation_no_match, captured false), with no
          internal decision. The accepted detector tradeoff is unchanged:
          matches call the classifier twice inside the legacy handler path —
          once through OWNER_DELEGATION_MATCHER.match and once to materialize
          rich intent — while the adapter performs no third match/detect;
          no-match calls only the matcher detector. RouteContext and
          RouteOutcome remain frozen. EXPECTED_PRE_BRAIN_ORDER deliberately
          removes the direct owner call (15->14); B45a/B45b/B45d/B44e rails
          are re-based without changing matcher/classifier semantics.'
    chokepoints:
      - bot.BotService.__init__  # _owner_delegation_slot construction
      - bot.BotService._route_owner_delegation
      - bot.BotService._handle_text_body
      - bot.BotService._maybe_handle_owner_delegation_request  # rich legacy handler
    enforced_by:
      - tests/test_b45e_owner_delegation_route_registry.py
      - tests/test_b44e_owner_delegation_matcher_pilot.py
      - tests/test_b45a_cleanup_route_registry.py
      ...
      - tests/test_telegram_imperative_router.py::test_owner_delegation_never_falls_back
      - tests/test_telegram_imperative_router.py::test_english_owner_delegation_with_options_never_falls_back
    why: Fifth per-slot migration proves the registry bridge preserves a
         handler that intentionally emits its own rich dynamic decision in
         addition to the registry static decision. Cardinality, order and
         reason ownership are locked so migration cannot silently collapse or
         duplicate owner-delegation telemetry.

  b45f_actionable_task_is_registry_invoked:
    rule: 'B4.5f. ACTIONABLE_TASK_MATCHER is invoked through registry Route
          data ONLY in the _handle_text_body text chain: dispatch_routes(
          self._actionable_task_slot, route_ctx,
          on_decision=self._emit_route_decision) runs exactly once at the
          ORIGINAL §5.1 row 8 slot, after owner delegation and telegram
          imperative and before F4 deterministic delegation. The single Route
          is Route(ACTIONABLE_TASK_MATCHER.name,
          self._route_actionable_task); it must NOT join _pre_brain_routes.
          The pure adapter passes ctx.stripped, ctx.session_id,
          ctx.runtime_channel and ctx.metadata["semantic_turn"] to the legacy
          handler. The existing RouteContext.metadata bag transports the
          already-classified semantic_turn without changing RouteContext or
          RouteOutcome. None becomes fall_through with the matcher unmatched
          slug; capture becomes intercepted with the matcher matched slug and
          store_memory_limit=2000. The registry callback emits exactly one
          static decision for match or no-match, without matched_pattern or a
          dynamic reason. Capture then preserves the direct decision -> store
          -> remember -> return path using the outcome limit, with no quality
          guard, final render or _post_capture_intercepted helper. Runtime,
          slash, session state, stateful follow-up, continuation resolution,
          preflight and execution remain inside the legacy handler. Both
          ACTIONABLE_TASK_MATCHER.match(text) consumers remain literal in the
          handler and objective resolver. EXPECTED_PRE_BRAIN_ORDER deliberately
          removes the direct actionable call (14->13).'
    chokepoints:
      - bot.BotService.__init__  # _actionable_task_slot construction
      - bot.BotService._route_actionable_task
      - bot.BotService._handle_text_body
      - bot.BotService._maybe_handle_actionable_task_request
      - bot.BotService._resolve_actionable_task_objective
    enforced_by:
      - tests/test_b45f_actionable_task_route_registry.py
      - tests/test_b44f_actionable_task_matcher_pilot.py
      - tests/test_actionable_task_matcher.py
      - tests/test_botservice_migration_rails.py
      - tests/test_telegram_imperative_router.py::test_owner_delegation_never_falls_back
    why: Sixth per-slot migration carries an already-computed semantic object
         through the frozen RouteContext metadata bag while leaving every
         stateful and executable decision in the legacy handler.
...
| 8 | actionable task (registry bridge) | runtime=Telegram and every slash/session/follow-up/continuation/preflight/execution decision remain in the legacy `_maybe_handle_actionable_task_request`; direct literal recognizer = declarative `ACTIONABLE_TASK_MATCHER` data (B4.4f, invariant `b44f_actionable_task_matcher_is_declarative_data`) with byte-identical `telegram_actionable_task` slugs. Since B4.5f only invocation is registry-backed here via `dispatch_routes(self._actionable_task_slot, route_ctx, on_decision=self._emit_route_decision)` (invariant `b45f_actionable_task_is_registry_invoked`), never in `_pre_brain_routes`. The existing `RouteContext.metadata` transports `semantic_turn`; capture keeps decision → store(limit 2000) → remember → return, with no quality guard/final render/matched_pattern/dynamic reason. Exact order remains owner < telegram imperative < actionable bridge < F4; owner wins the "hazlo tú" overlap |
