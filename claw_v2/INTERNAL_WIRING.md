...
verification_method: "TWO slices this session. (1) secret-path-upload-sigil (audit CRITICAL-1): TDD red-first — test_curl_form_upload_secret_path_blocked failed pre-fix on all 3 field=@path forms (PermissionError not raised — the vuln, auto-exec), green post; + test_curl_upload_nonsecret_workspace_path_still_allowed guard. New invariant bash_secret_path_denylist_covers_upload_sigil. (2) evidence-lane-sonnet5: web-evidence pre-step gets role coordinator_evidence (env CLAW_EVIDENCE_PROVIDER/MODEL, default=worker mirror), TDD red-first — 3 config tests + test_evidence_worker_uses_dedicated_evidence_role failed pre (role='coordinator_worker'), green post. New invariant evidence_pre_step_role_scoped_model. No regressions: test_runtime_policy 25, config+coordinator 136, architecture_invariants+task_handler+llm 135, secret_scanning green. My changed lines ruff-clean (pre-existing F541/F841 + coordinator.py format drift at 1994/2326 are NOT from this slice — left untouched per surgical rule). LIVE SMOKE PASS (2026-07-03, PR #193 merged ff98e49, deploy clone ~/srv/claw-daemon pid 29613): clean boot — new incarnation with both fixes in the loaded tree, port 8765 LISTEN, observe_stream writing fresh, stderr boot delta 0 tracebacks, watchdog no flap (pid stable across the smoke). Task 1 non-regression against the DEPLOYED tree + production policy (network_policy='allow'): both curl field=@secret forms DENIED ('command references a secret path'), benign multipart upload allowed (no over-block). Task 2 against the deployed config: coordinator_evidence → anthropic/claude-sonnet-5, coordinator_worker → codex/gpt-5.5 (code unmoved). OPEN LANES (not driven through inbound brain surface): (1) Task 1 denial not driven via a live brain Bash turn — staging a secret-exfil prompt on the production agent is inappropriate; close on the next organic curl-upload attempt or a controlled test in an isolated daemon. (2) Task 2 role not exercised live — coordinator_evidence only fires on the redrive→evidence-gate pre-step (run_evidence_worker), impractical to stage on demand; close by observing the next organic redrive-with-evidence task's gather_evidence worker resolving to claude-sonnet-5 in observe_stream. Predecessor: slice critical-echo (doc_version 2.66) in main 8ce4a60."
