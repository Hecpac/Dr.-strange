...
```yaml
describes_commit: "slice open-url-task-fallthrough (2026-07-03): the pre-brain URL shortcut (_maybe_handle_shortcut, bot.py) over-captured 'Abre <URL> ahí está el prototipo original' — a single-URL open command carrying task content — and ran a shallow authenticated-CDP text browse instead of letting the brain plan the real task, so the reply read like it saw the design without doing it (live: messages 3426/3427, dispatch captured=shortcut, brain fell through). Fix (Option A): new _open_command_carries_task (bot_helpers.py) routes such messages to the brain (>= 2 content words beyond URL+open-verb → _BrainShortcut), mirroring the pre-existing multi-URL -> brain rule; bare/duplicate single URLs stay on the CDP shortcut. Predecessor: slice ssrf-nested-follow (doc_version 2.69) in main 5fbe1b4 / docs 8493695. --- prior: slice ssrf-nested-follow (2026-07-03, audit #2): BrowseHandler._append_nested_url_reviews (browse_handler.py) auto-followed up to 3 links found in a fetched page with the LOCAL browser and no IP guard — attacker-plantable input. A page linking to 169.254.169.254 (cloud IMDS) / 127.0.0.1:9250 (Chrome CDP) made the daemon fetch internal resources and fold them into the reply (SSRF), reachable from a normal Telegram turn. The existing DomainAllowlistEnforcer/_enforce_resolved_ips (is_global check, network_proxy.py) guarded only the ToolRegistry path, never browse_handler. Fix (Option B): each nested URL now passes enforce_url(allowed_domains=[]) before browse_response fetches it; non-public IPs are surfaced as '[URL anidada bloqueada por seguridad]', never fetched. Nested-follow only — the operator's primary browse URL stays unguarded by design (localhost browsing preserved). Predecessors: slices secret-path-upload-sigil + evidence-lane-sonnet5 (doc_version 2.68) in main ff98e49; docs b69fa52."
doc_version: 2.74
last_verified: 2026-07-05
verification_method: "slice open-url-task-fallthrough (2026-07-03, live bug messages 3426/3427): the pre-brain shortcut intercepted 'Abre <URL> ahí está el prototipo original' (captured=shortcut, brain fell through), did a shallow authenticated-CDP TEXT browse of claude.ai/design (~11s) and dumped the dashboard — looked like it saw the design but never opened the prototype / did the task. Root: bot.py open-verb branch sent single-URL to _chrome_handler.browse_response instead of the brain, even when the message carried task content. Fix (Option A): _open_command_carries_task (bot_helpers.py) — single-URL open command with >= 2 content words beyond URL+verb falls through to the brain. TDD red-first: test_open_url_with_task_clause_falls_through_to_brain failed pre-fix (returned CDP-OK), green post; test_bare_open_command_still_routes_to_cdp guards bare/duplicate URLs stay on CDP (caught that _extract_url_candidates returns SCHEME-normalized URLs that don't string-match a bare host — switched to _SCHEME_URL_RE/_HOST_URL_RE strip). No regressions: test_browse 36 + test_telegram_imperative_router + test_dispatch_route/routing + test_bot_helpers 169 passed; ruff clean. Residual: >= 2-word threshold is a heuristic. New invariant open_url_with_task_intent_falls_through_to_brain. LIVE SMOKE PASS (2026-07-03, PR #197 merged 63cfb60, deploy clone ~/srv/claw-daemon pid 64262): clean boot for the change — port 8765 LISTEN, observe_stream fresh, pid stable, ZERO tracebacks/RuntimeDatabaseError/branch-integrity in the boot delta, no watchdog flap. Routing exercised against the DEPLOYED tree: OpenSiteCdpRoutingTests green on the clone — the 'Abre <URL> ...prototipo original' clause returns _BrainShortcut (falls to brain), bare/duplicate URLs stay CDP. OPEN LANE: the full inbound Telegram e2e was NOT driven live (would post a user-facing turn to the owner's production chat); close on the owner's next such message reaching the brain in observe_stream. UNRELATED pre-existing errors observed in the boot delta (not this change): BrowserSession ConnectionRefused on 127.0.0.1:9250 from a resumed background X-scrape task — Chrome CDP not running; flagged, not this slice's scope. Predecessor: doc_version 2.69 (slice ssrf-nested-follow) in main 5fbe1b4 / docs 8493695. --- prior: slice ssrf-nested-follow (audit #2, validated GENUINE + independently re-confirmed this session): BrowseHandler._append_nested_url_reviews auto-followed attacker-plantable links with the local browser and no IP guard, so a page linking to 169.254.169.254 (cloud IMDS) / 127.0.0.1:9250 (Chrome CDP) exfiltrated internal resources into the reply — reachable from a normal Telegram turn. Fix (Option B, owner-chosen): each nested URL passes DomainAllowlistEnforcer.enforce_url(allowed_domains=[]) → rejects non-public IPs BEFORE browse_response fetches; primary operator URL stays unguarded (localhost browsing preserved). TDD red-first: test_nested_non_public_ip_is_blocked_not_fetched failed pre-fix (169.254 WAS fetched — the vuln), green post; test_all_public_nested_urls_still_followed guards no over-block (hermetic, literal IPs — caught that RFC5737 198.51.100.0/24 is is_global=False, used 8.8.8.8). New invariant browse_nested_follow_blocks_non_public_ip. No regressions: test_browse+test_browser+architecture_invariants 90 passed/22 subtests; ruff clean on changed lines. Residuals named (not closed): redirect/DNS-rebinding, external-Jina text path. Prior slices secret-path-upload-sigil + evidence-lane-sonnet5 LIVE SMOKE PASSED (PR #193/#194, clone pid 29613). LIVE SMOKE PASS (2026-07-03, PR #195 merged 5fbe1b4, deploy clone ~/srv/claw-daemon pid 49823): clean boot — deployed browse_handler in the loaded tree, port 8765 LISTEN, observe_stream fresh, stderr boot delta 0 tracebacks, pid stable. Guard exercised against the DEPLOYED tree: NestedFollowSsrfGuardTests green on the clone; enforce_url blocks 169.254.169.254 + 127.0.0.1:9250 ('Host resolves to a non-public IP address'), allows 8.8.8.8. OPEN LANE (not driven through inbound Telegram surface): the end-to-end fetch of an attacker page with a planted internal-IP link was not staged live (would require hosting such a page); close on the next organic browse of a page carrying a non-public-IP link, confirming the '[URL anidada bloqueada por seguridad]' block in observe_stream. Predecessor: doc_version 2.68 in main ff98e49 / docs b69fa52."
