...
invariants:
  ...
  audit_critical_observe_events_survive_contention:
    rule: Audit-critical observe events are classified centrally in
          ...
          tool-use, runtime-policy, auth, RuntimeDb degradation, branch
          integrity, scheduler-error, and critical failure events must carry
          `audit_critical=true` in their payload. If RuntimeDb contention or a
          shared-connection write error prevents immediate insertion, the spill
          record must also carry `audit_critical=true` so recovery and review
          can distinguish audit evidence from ordinary diagnostics. Non-critical
          observe events remain non-blocking and are not marked as audit-critical
          by default.
    enforced_by:
