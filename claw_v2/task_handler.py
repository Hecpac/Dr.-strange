...
def _evidence_pre_step_instruction(verdict: str) -> str:
    ...
    return (
        ...
        "head, plutil -p, grep, sed -n). Nada de red, nada de escritura, nada "
        "de instalación.\n"
        ...
        "- Si la cita es una sección/entrada de un documento largo, localiza y "
        "pega SOLO la sección pedida con su contexto inmediato (grep -n para "
        "ubicarla, sed -n para extraerla) — NUNCA el documento desde el inicio: "
        "tu salida es limitada y la sección quedaría fuera.\n"
        "- Para man pages: `man <página> | col -b` y extrae la entrada pedida; "
        "si man no renderiza en tu sandbox, `man -w <página>` + extracción de "
        "la sección del fuente con sed -n/grep.\n"
