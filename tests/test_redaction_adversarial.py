...
SHOULD_REDACT = [
    ("fake anthropic key body 36+ chars",
     "sk-ant-api03Abc123Xyz789Def456GhiJkl0987"),
    ("anthropic admin key fake",
     "AbcDefGhi123JklMnoPqr456StuVwxYza789ZzAa"),
    ("base64 jwt-style 40+ chars mixed",
     "eyJhbGciOiJIUzI1NiJ9Abc123Def456GhiJkl789Mno"),
    ("high entropy 40 chars",
     "AbcDef123GhiJkl456MnoPqr789StuVwx012YzAb"),
]
...
SHOULD_NOT_REDACT = [
    ...
    ("git commit hash 40 hex lowercase",
     "eecd358abc1234567890def1234567890abc12d4"),
    ("sha256 lowercase hex 40 chars",
     "9ea02a6d43dc4ce7970b1c636c560904f62a799b"),
    ("uuid without hyphens lowercase",
     "f81d4fae7dec11d0a76500a0c91e6bf6"),
    ("uuid with hyphens lowercase",
     "81bbc5d7-3eb5-4cb0-8a93-b74aed8ce1af"),
    ("uuid hyphen mixed UPPER",
     "5d6282ce-54d0-4F5E-8bb7-2164436b375b"),
    ("parcel id short", "76092-0001"),
    ("roboflow model slug", "swimming-pools-dctlb"),
    ("branch name", "feat/tactical-autonomy-fixes"),
    ("telegram task id",
     "tg-574707975:1778533984299303000"),
    ...
    ("file path with hash",
     "/Users/hector/Projects/Dr.-strange/.git/objects/ee/cd358abc"),
    ("known gap: 20-char roboflow key (caught only with prefix context)",
     "8eyt8R1Hp008liTCA98a"),
    ("camelcase 32+ chars but only 1 digit",
     "thisIsAVeryLongCamelCaseIdentifier1"),
]
...
def test_should_redact(name, specimen):
    ...
    assert "[REDACTED]" in out or "<REDACTED" in out, (
        f"[{name}] expected redaction, got: {out!r}"
    )
...
def test_no_catastrophic_backtracking_on_pathological_input():
    ...
    assert elapsed < 0.5, (
        f"regex took {elapsed:.2f}s on 1200-char pathological input"
    )
...
def test_telegram_task_id_passes_through_intact():
    ...
    assert "tg-574707975:1778533984299303000" in out, (
        f"task id got mangled: {out!r}"
    )
