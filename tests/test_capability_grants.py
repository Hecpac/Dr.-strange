...
class CapabilityGrantStoreTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.store = CapabilityGrantStore(
            Path(self._tmp.name) / "claw.db", observe=self.observe
        )
    ...
    def test_grant_rejects_invalid_scope_kind(self) -> None:
        with self.assertRaises(ValueError):
            self.store.grant(
                GrantScope(kind="nuke", target="*"), grantor="user"
            )
    ...
    def test_grant_without_ttl_is_active_indefinitely(self) -> None:
        record = self.store.grant(
            GrantScope(kind="domain", target="x.com"), grantor="user"
        )
        self.assertIsNone(record.expires_at)
        self.assertTrue(record.is_active())
        self.assertTrue(
            self.store.is_granted(kind="domain", target="x.com")
        )
    ...
    def test_revoke_marks_inactive_and_emits_event(self) -> None:
        record = self.store.grant(
            GrantScope(kind="tool", target="Write"), grantor="user"
        )
    ...
    def test_revoke_already_revoked_returns_none(self) -> None:
        record = self.store.grant(
            GrantScope(kind="tool", target="Bash"), grantor="user"
        )
    ...
    def test_find_grants_for_returns_all_matches(self) -> None:
        ...
        g2 = self.store.grant(
            GrantScope(kind="tool", target="*"), grantor="kairos"
        )
        self.store.grant(
            GrantScope(kind="tool", target="Write"), grantor="user"
        )
    ...
    def test_list_active_ignores_revoked_and_expired(self) -> None:
        active_grant = self.store.grant(
            GrantScope(kind="tool", target="Read"), grantor="user"
        )
