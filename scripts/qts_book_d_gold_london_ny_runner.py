"""Book D paper trading runner — XAUT/USDT 1h TF — London+NY extended window.

Bridge from Dr.-strange workspace into QTS-ARCHITECT runtime. Runs in
parallel with live A/B (1h NY 8-11) and Book C (15m NY 8-11).

Only changes vs A/B:
- Session window: NY 3-13 local (≈ UTC 7-17 DST) — captures London open through
  NY close lunch, ~10h daily vs A/B's 3h.
- Same TF (1h), same symbol (XAUT/USDT OKX), same Sprint 6 guardrails.

Window aligned with A/B T0 (2026-05-11) so per-day comparisons stay valid.
Exploratory: no promotion gate, no B-book control.

State: ~/Projects/QTS-ARCHITECT/state/paper_book_d_gold_london_ny.json
Reports: ~/Projects/QTS-ARCHITECT/reports/qts-paper-book-d-gold-london-ny-state-*

Run: ~/Projects/QTS-ARCHITECT/.venv/bin/python ~/Projects/Dr.-strange/scripts/qts_book_d_gold_london_ny_runner.py
"""
...
STATE_FILE = STATE_DIR / "paper_book_d_gold_london_ny.json"
...
async def main():
    ...
    log.info("book_d_run_start", t0=T0_ISO, until=until_iso, tf=TIMEFRAME,
             session=f"NY {SESSION_START_NY}-{SESSION_END_NY}")
    ...
    state = {
        "spec": "qts-book-d-gold-london-ny-exploratory",
        ...
        "symbol": SYMBOL,
        "timeframe": TIMEFRAME,
        "session_window_ny": f"{SESSION_START_NY}-{SESSION_END_NY}",
        "initial_capital": INITIAL_CAPITAL,
        "trade_size": TRADE_SIZE,
    ...
    snap_json = REPORTS_DIR / f"qts-paper-book-d-gold-london-ny-state-{date_str}.json"
    snap_json.write_text(json.dumps(state, indent=2, default=str))

    md = [
        f"# QTS Book D — Gold London+NY exploratory — state @ {now_utc.strftime('%Y-%m-%d %H:%M UTC')}",
        "",
        ...
        f"- Bars consumed: {len(df)}  ·  TF: {TIMEFRAME}  ·  Session: NY {SESSION_START_NY}-{SESSION_END_NY}",
        "",
        "## Book D (London+NY extended exploratory)",
    ...
    snap_md = REPORTS_DIR / f"qts-paper-book-d-gold-london-ny-state-{date_str}.md"
    snap_md.write_text("\n".join(md))

    print(json.dumps({
        "status": "ok",
        "days_elapsed": state["diff"]["days_elapsed"],
        "book_D": {"capital": pnl["capital"], "return_pct": round(ret_pct, 4),
