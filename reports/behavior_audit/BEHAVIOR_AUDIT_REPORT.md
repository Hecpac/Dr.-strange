...
- Casos sanitizados generados: **524**.
- El patrón dominante de riesgo operativo es `completed_unverified` / `needs_verification`: **91** casos etiquetados.
- Tool use observado: **502** eventos/casos con tools; principales tools abajo.
- Permisos: **36** casos pidieron aprobación; **209** aparecen como posibles permisos faltantes por heurística.
- Costo estimado agregado en la muestra: **$0.0**. Nota: OpenAI/Codex reportan costo `0.0` en adapters, por lo que esta métrica subestima costo API real.
- Latencia Telegram mediana: **80844.0 ms**; p95: **167679.8 ms** cuando había eventos `telegram_latency`.
...
| Posibles approvals faltantes | 209 |
| Completions no verificadas | 91 |
| Posibles false success | 0 |
| Costo estimado total | $0.0 |
| Mediana latencia ms | 80844.0 |
...
|---|---:|
| `intent_correct` | 484 |
| `missing_permission` | 209 |
| `good_tool_use` | 194 |
| `memory_good` | 138 |
| `unverified_completion` | 91 |
| `wrong_tool` | 53 |
| `intent_missed` | 40 |
| `unnecessary_permission` | 30 |
| `memory_bad` | 23 |
| `good_workflow_candidate` | 15 |
...
|---|---:|
| `tool_call` | 200 |
| `task_ledger:telegram:brain_fallback:completed_unverified` | 91 |
...
- `tool_14327` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Read'], verification=`unknown`. Request: Tool event: Read; Outcome: sdk_post_tool_use
- `tool_14492` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Write'], verification=`unknown`. Request: Tool event: Write; Outcome: sdk_post_tool_use
- `tool_14496` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Bash'], verification=`unknown`. Request: Tool event: Bash; Outcome: sdk_post_tool_use
- `tool_14497` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Read'], verification=`unknown`. Request: Tool event: Read; Outcome: sdk_post_tool_use
- `tool_14560` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Write'], verification=`unknown`. Request: Tool event: Write; Outcome: sdk_post_tool_use
- `tool_14563` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Bash'], verification=`unknown`. Request: Tool event: Bash; Outcome: sdk_post_tool_use
- `tool_14606` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Write'], verification=`unknown`. Request: Tool event: Write; Outcome: sdk_post_tool_use
- `tool_14608` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Edit'], verification=`unknown`. Request: Tool event: Edit; Outcome: sdk_post_tool_use
- `tool_14932` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Write'], verification=`unknown`. Request: Tool event: Write; Outcome: sdk_post_tool_use
- `tool_14933` — intent=`sdk_post_tool_use`, task=`tool_call`, tools=['Bash'], verification=`unknown`. Request: Tool event: Bash; Outcome: sdk_post_tool_use
...
- `task_d2ce6bf49df1` — intent=`task_ledger`, task=`task_ledger:telegram:brain_fallback:completed_unverified`, tools=['Bash', 'Write'], verification=`needs_verification`. Request: Extrae el video; Outcome: brain tool-use completed with warnings: 2 substep failure(s)
- `task_67f1ee6fee3c` — intent=`task_ledger`, task=`task_ledger:telegram:brain_fallback:completed_unverified`, tools=['Write'], verification=`needs_verification`. Request: Mañana publicamos; Outcome: brain tool-use turn: 3 tool calls (unverified)
...
| Posibles faltantes | 209 por high-risk tool sin approval visible |
...
Hallazgo: **91** casos quedan con etiqueta `unverified_completion`. Esto no significa necesariamente fallo, pero sí deuda operacional.
...
Casos candidatos detectados: **15**.
...
- `task_e57f138bae49` — intent=`task_ledger`, task=`task_ledger:telegram:brain_fallback:completed_unverified`, tools=['Bash', 'Edit', 'Read', 'Write'], verification=`needs_verification`. Request: Revisa la carpeta de descarga ahí pude el audio; Outcome: brain tool-use completed with warnings: 5 substep failure(s) — Opportunity: Review/audit pattern: candidate for a reusable evidence-first review workflow.
